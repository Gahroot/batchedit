...
Electron + React 19 desktop app that mass-produces short-form video ads by taking clips in three buckets (Hook, Meat, CTA), generating the Cartesian product `hooks × meats × ctas`, and batch-rendering each hook→meat→cta concatenation with FFmpeg, optional burned-in Whisper captions, text/image overlays, and an optional autonomous Gemini agent that drives the whole pipeline.
...
- `src/main/agent/` — autonomous LLM agent (`@prestyj/agent` + Gemini) that runs ingest→transcribe→detect markers→split→analyze→pick template→render with human review gates. Tools are one-per-file under `agent/tools/`.
...
- **The agent runs in main but needs renderer capabilities** (Whisper, store, rendering). It calls back via `agentBridge` RPC (`agent:transcribe` / `agent:getStoreSnapshot` / `agent:startRender`) fulfilled by renderer hooks `useAgent*Bridge`.
...
- Secrets: the Gemini API key is used by `src/main/agent/` — keep it out of committed code and logs.
